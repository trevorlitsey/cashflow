...
import { message, Form, Input, InputNumber, DatePicker, Select, Icon, Button, Switch } from 'antd';
import moment from 'moment';

import { Container, Grid, IntervalSelect, SwitchContainer } from './StyledComponents';
...
class NewRecurringExpenseForm extends React.Component {
	...
	handleSubmit = (e) => {
		e.preventDefault();
		const { startDate, name, amount, frequency, interval, isRecurring } = this.state;
		...
		if (isRecurring && (!frequency || !interval)) {
			return message.error('frequency and interval are required to set recurring expense')
		}
	...
	render() {
		...
		return (
			<Container isRecurring={isRecurring}>
				<SubSubTitle>Add new income/expense:</SubSubTitle>
